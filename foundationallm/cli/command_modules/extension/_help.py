...
parameters:
  - name: --system
    ...
    long-summary: Default path is fllm-cli-extensions folder under the CLI running python environment lib path, configurable by environment variable AZURE_EXTENSION_SYS_DIR. On Windows, you may need to open your shell as Administrator to run with the right permission.
examples:
  - name: Add extension by name
    text: fllm extension add --name anextension
  - name: Add extension from URL
    text: fllm extension add --source https://contoso.com/anextension-0.0.1-py2.py3-none-any.whl
  - name: Add extension from local disk
    text: fllm extension add --source ~/anextension-0.0.1-py2.py3-none-any.whl
  - name: Add extension from local disk and use pip proxy for dependencies
    text: fllm extension add --source ~/anextension-0.0.1-py2.py3-none-any.whl --pip-proxy https://user:pass@proxy.server:8080
  - name: Add extension to system directory
    text: fllm extension add --name anextension --system
  - name: Add a specific version of extension
    text: fllm extension add --name anextension --version 1.0.0
  - name: Upgrade the extension if already installed
    text: fllm extension add --upgrade --name anextension
"""
...
examples:
  - name: List all publicly available extensions
    text: fllm extension list-available
  - name: List details on a particular extension
    text: fllm extension list-available --show-details --query anextension
"""
...
examples:
  - name: Remove an extension. (autogenerated)
    text: fllm extension remove --name MyExtension
    crafted: true
"""
...
examples:
  - name: Show an extension. (autogenerated)
    text: fllm extension show --name MyExtension
    crafted: true
"""
...
examples:
  - name: Update an extension by name
    text: fllm extension update --name anextension
  - name: Update an extension by name and use pip proxy for dependencies
    text: fllm extension update --name anextension --pip-proxy https://user:pass@proxy.server:8080
"""
...
examples:
  - name: List available versions for an extension
    text: fllm extension list-versions --name anextension
"""
